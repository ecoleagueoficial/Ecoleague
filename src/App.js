...
export default function EcoQuest() {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      // ✅ Llamada DIRECTA a iNaturalist sin proxy
      ...
      if (!res.ok) {
        ...
        console.error("iNaturalist error:", res.status, errText);
      ...
      if (top && top.taxon) {
        ...
        const isInvasora = taxon.establishment_means &&
          taxon.establishment_means.establishment_means === "introduced";
        ...
        const parsed = {
          ...
          curiosity: "Esta especie ha sido observada " +
            (taxon.observations_count ? taxon.observations_count.toLocaleString() : "miles de") +
            " veces en todo el mundo.",
          imageUrl: taxon.default_photo && taxon.default_photo.medium_url,
        };
      ...
      setResult({ type: "error", message: "Error de red al analizar la imagen. Comprueba tu conexión." });
    }

    setScanning(false);
    // Reset input para poder subir la misma foto de nuevo
  ...
  const nextPlantAch = [
    { need: 1, id: "first_plant" },
    { need: 5, id: "plants_5" },
    { need: 10, id: "plants_10" },
    { need: 20, id: "plants_20" },
  ].find(a => !unlockedAchievements.includes(a.id));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0a1628 0%,#0d2218 50%,#0a1628 100%)", fontFamily: "Georgia,serif", color: "#e8f5e9", position: "relative", overflow: "hidden" }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .recycle-card:active { transform: scale(0.95); }
      `}} />
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 0 100px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ padding: "32px 24px 20px", textAlign: "center" }}>
          <div style={{ fontSize: 13, letterSpacing: 4, color: "#4ade80", textTransform: "uppercase", marginBottom: 8 }}>Proyecto</div>
          ...
          {showNameInput && (
            <div style={{ marginTop: 10, display: "flex", gap: 8, justifyContent: "center" }}>
              <input
                ...
                onKeyDown={e => { if (e.key === "Enter") { setUserName(tempName); setShowNameInput(false); } }}
              ...
              />
              <button onClick={() => { setUserName(tempName); setShowNameInput(false); }} style={{ background: "linear-gradient(135deg,#16a34a,#0d9488)", color: "white", border: "none", borderRadius: 50, padding: "8px 16px", fontSize: 13, cursor: "pointer" }}>Guardar</button>
            </div>
          )}
        </div>
        ...
        <div style={{ margin: "0 20px 20px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {[
            { label: "Mis puntos", val: myPoints, emoji: "⭐", color: "#fbbf24" },
        ...
        {/* Barra de progreso siguiente logro */}
        {nextPlantAch && (
          <div style={{ margin: "0 20px 20px", background: "linear-gradient(135deg,rgba(251,191,36,0.12),rgba(251,191,36,0.06))", borderRadius: 16, padding: "12px 16px", border: "1px solid rgba(251,191,36,0.25)", display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "#86efac88", marginTop: 2 }}>
                {uniquePlants >= nextPlantAch.need ? "¡Completado!" : `Encuentra ${nextPlantAch.need - uniquePlants} especies más`}
              </div>
              <div style={{ marginTop: 6, height: 4, background: "rgba(255,255,255,0.1)", borderRadius: 2 }}>
                <div style={{ height: "100%", borderRadius: 2, width: Math.min((uniquePlants / nextPlantAch.need) * 100, 100) + "%", background: "linear-gradient(90deg,#fbbf24,#f59e0b)", transition: "width 0.5s ease" }} />
              </div>
        ...
        <div style={{ display: "flex", margin: "0 20px 24px", background: "rgba(0,0,0,0.3)", borderRadius: 16, padding: 4, gap: 4 }}>
          {[
            { id: "scan", label: "📸 Plantas" },
            { id: "recycle", label: "♻️ Reciclar" },
            { id: "achievements", label: "🏆 Logros" },
            { id: "league", label: "🥇 Liga" },
          ].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "10px 2px", borderRadius: 12, border: "none", background: tab === t.id ? "linear-gradient(135deg,#16a34a,#0d9488)" : "transparent", color: tab === t.id ? "white" : "#86efac88", fontSize: 10, fontWeight: tab === t.id ? "700" : "400", cursor: "pointer", boxShadow: tab === t.id ? "0 4px 12px #16a34a44" : "none" }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === "scan" && (
          <div style={{ padding: "0 20px" }}>
            ...
            <div
              ...
              style={{ background: "rgba(255,255,255,0.04)", borderRadius: 24, padding: 28, border: "2px dashed rgba(74,222,128,0.3)", textAlign: "center", marginBottom: 24, cursor: "pointer", position: "relative" }}
            >
              <input
                ref={fileInputRef}
                type="file"
              ...
              {scanning ? (
                <div>
                  {previewUrl && <img src={previewUrl} alt="preview" style={{ width: "100%", maxHeight: 180, objectFit: "cover", borderRadius: 16, marginBottom: 16, opacity: 0.6 }} />}
                ...
                <div>
                  <img src={previewUrl} alt="preview" style={{ width: "100%", maxHeight: 160, objectFit: "cover", borderRadius: 16, marginBottom: 12 }} />
                  <div style={{ fontSize: 13, color: "#4ade80" }}>📸 Toca para analizar otra foto</div>
                </div>
              ) : (
                <>
            ...
            {result && result.type === "plant" && (
              <div style={{ background: "linear-gradient(135deg,rgba(22,163,74,0.12),rgba(13,148,136,0.12))", borderRadius: 24, padding: 22, border: "1px solid rgba(74,222,128,0.3)", marginBottom: 24, animation: "fadeUp 0.4s ease" }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 14 }}>
                  {result.imageUrl ? (
                    <img src={result.imageUrl} alt={result.name} style={{ width: 64, height: 64, borderRadius: 14, objectFit: "cover", flexShrink: 0 }} />
                  ) : (
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 10, marginTop: 8, flexWrap: "wrap" }}>
                      ...
                      <span style={{ fontSize: 11, color: result.confidence === "Alta" ? "#4ade80" : result.confidence === "Media" ? "#fbbf24" : "#f87171" }}>● Confianza {result.confidence}</span>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
                  <div style={{ background: "rgba(74,222,128,0.08)", borderRadius: 14, padding: "12px 14px", gridColumn: "1/-1" }}>
                    <div style={{ fontSize: 11, color: "#4ade80", fontWeight: "700", marginBottom: 5 }}>🌿 USOS</div>
                    <div style={{ fontSize: 13, color: "#cce5cc", lineHeight: 1.5 }}>{result.uses}</div>
                  </div>
                  <div style={{ background: "rgba(251,191,36,0.08)", borderRadius: 14, padding: "12px 14px" }}>
                    <div style={{ fontSize: 11, color: "#fbbf24", fontWeight: "700", marginBottom: 5 }}>🌸 FLORACIÓN</div>
                    <div style={{ fontSize: 13, color: "#cce5cc" }}>{result.flowering}</div>
                  </div>
                  <div style={{ background: "rgba(34,211,238,0.08)", borderRadius: 14, padding: "12px 14px" }}>
                    <div style={{ fontSize: 11, color: "#22d3ee", fontWeight: "700", marginBottom: 5 }}>🔬 CARACTERÍSTICAS</div>
                    <div style={{ fontSize: 12, color: "#cce5cc", lineHeight: 1.7 }}>
                      {result.characteristics.split(";").map((c, i) => <div key={i}>· {c.trim()}</div>)}
                    </div>
                  </div>
                </div>

                {result.curiosity && (
                  <div style={{ background: "rgba(168,85,247,0.1)", borderRadius: 14, padding: "12px 14px", border: "1px solid rgba(168,85,247,0.2)" }}>
                    <div style={{ fontSize: 11, color: "#c084fc", fontWeight: "700", marginBottom: 5 }}>✨ DATO CURIOSO</div>
                    <div style={{ fontSize: 13, color: "#cce5cc", fontStyle: "italic" }}>{result.curiosity}</div>
                  </div>
                )}
              </div>
            )}
          ...
          <div style={{ padding: "0 20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {RECYCLE_ITEMS.map(item => (
                <div
                  ...
                  style={{ background: "rgba(255,255,255,0.05)", borderRadius: 20, padding: "20px 16px", textAlign: "center", border: "1px solid " + item.color + "44", cursor: "pointer", transition: "transform 0.1s ease" }}
                >
            ...
            <div style={{ marginTop: 24, background: "rgba(255,255,255,0.04)", borderRadius: 20, padding: 20, border: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              {Object.entries(recycleCount).length === 0 ? (
                <div style={{ color: "#86efac88", fontSize: 13 }}>Aún no has registrado nada</div>
                ...
                return (
                  <div key={id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    ...
                    <span style={{ color: item.color, fontWeight: "700" }}>{count}x · {count * item.points} pts</span>
                  </div>
                );
              })}
            </div>
